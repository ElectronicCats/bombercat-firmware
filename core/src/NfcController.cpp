...
bool NfcController::receiveNoGarbage(uint8_t *pData, uint8_t *pDataSize,
  ...
  // (reads only when the PN7150 raises IRQ HIGH, i.e. when the CHIP has data
  // for us, which our write does not request). Dropping it reclaims ~23 ms per
  ...
  // command. Keep reading (IRQ-gated) and IGNORE any non-data frame until a
  // real data packet arrives or toutMs elapses. On timeout, pData/pDataSize are
  // left untouched so callers keep any earlier value as a safe fallback.
  ...
  while (millis() - start < toutMs) {
    uint32_t n =
        _nfc.readData(buf); // public primitive; reads only when IRQ HIGH
    if (!n)
      continue; // no frame yet — keep polling within toutMs
...
bool NfcController::beginEmulationMode() {
  ...
  // Re-arm discovery now that the mode is EMULATION. This mirrors the
  // known-good legacy sequence `nfc.setEmulationMode(); resetMode();`
...
bool NfcController::readerTransceive(uint8_t *cmd, uint8_t cmdLen,
  ...
  // data. That broke every transaction at GPO (the first non-SELECT command)
  // and then tripped the destructive mid-transaction re-arm in
  // readerHandleCommand.
  if (!receiveNoGarbage(resp, respLen, timeoutMs))
    return false;
  ...
  // RECORD). This directly contradicts docs/NFCGATE_PLAN.md §17's assumption
  // that the second receive never hits the 2000 ms ceiling — hardware shows it
  // does.
  //
  // IRQ-gate it: the PN7150 drives its IRQ line HIGH only when a frame is
  // pending, so poll hasMessage() for a short window. If a genuine second
  // packet arrives (two-packet cards — fast, well under this window per §17's
  // ~450 ms legs) read it as the real answer; if the window elapses with no IRQ
  ...
  receiveNoGarbage(resp, respLen,
                   50); // genuine second data packet = the answer
  return true;
}
...
bool NfcController::cardReceive(uint8_t *buf, uint8_t *len) {
  ...
  // immediately when the terminal actually sends). Returns true on a data
  // frame.
...
bool NfcController::cardSend(uint8_t *buf, uint16_t len) {
  ...
  // packets and set the Packet Boundary Flag (PBF, bit 4 of the header) on
  // every segment except the last, so the PN7150 reassembles them into one RF
  // frame. Header layout matches the library's cardModeSend():
  // [conn-id/PBF][RFU=0][len].
  //
  ...
  // here (aborting the send + logging) turned that harmless hiccup into a
  // visible failure and a Camino A regression — so for a <=255 B response this
  // is now byte-for-byte the same I2C sequence as the known-good
  // cardModeSend().
...
bool NfcController::cardReArm() {
  ...
  // + a final reset — mode is already EMULATION here, so no RW detour). It
  // costs a chip re-init (~tens of ms) but only runs once per terminal
  // departure.
