 ...
 * carrying any APDUs on the wire (APDUs go over WiFi/TCP — see
 * docs/NFCGATE_PLAN.md).
 *
 ...
 *                      "+OK [text]"       command succeeded (terminates a
 * reply)
 ...
 *                                (state goes idle -> connecting ->
 * relaying|error). stop                      -> +OK          stop the relay
 *   status                    -> :state :detail :connected :peer :relayed +OK
 *                                state: idle|connecting|relaying|error;
 *                                detail: current phase or last error (human
 * text) identify                  -> +OK           blink the LED for a couple
 * of seconds so the user can tell which physical board a CLI device ID refers
 * to. Returns at once; the blinking runs from the sketch loop(). capture
 * <on|off>          -> +OK capture on|off   arm/disarm the APDU tap. While on,
 * the engine copies each relayed APDU out as ":apdu <dir> <ts_ms> <hex>"
 * (dir=cmd|resp) on this stream — the host turns it into a pcap for Wireshark
 * (Fase 8). Bare `capture` or `capture status` -> :capture <0|1> +OK. reboot ->
 * +OK, then resets the MCU
 *
 * The relay actions (run/stop/reboot) are provided by the sketch as callbacks
 * so this class — and all of core/ — stays free of any WiFiNINA dependency (the
...
public:
  ...
  struct Callbacks {
    // Kick off the (non-blocking) relay bring-up. Returns nullptr when
    // accepted, or a short human error string (reported as -ERR) when it can't
    // start (e.g. empty SSID, already running). Must return promptly — it only
    // starts the bring-up; the sketch's loop() advances it and `status` reports
    // it.
    ...
    // Control-plane state name: "idle"|"connecting"|"relaying"|"error". Owned
    // by the sketch because the "connecting" sub-phases (WiFi/NFC/TCP/SYN) live
    // there. If null, `status`/`info` fall back to the engine's own state.
    const char *(*state)() = nullptr;
    // Optional human-readable detail (current phase or last error). If null,
    // the `status` reply omits the :detail line.
  ...
  // Read whatever bytes are available and dispatch each complete line. Call
  // once per loop(); never blocks.
  void poll();
