/*** ⚠️ OBSOLETO / LEGACY ────────────────────────────────────────────────────
 * Sketch Relay NFC (rol READER, transporte MQTT). Reemplazado por
...
// Get limits of the In Application Program (IAP) flash, ie. the internal MCU
// flash.
#include "FlashIAPLimits.h"
auto iapLimits{getFlashIAPLimits()};

// Create a block device on the available space of the FlashIAP
FlashIAPBlockDevice blockDevice(iapLimits.start_address,
                                iapLimits.available_size);
...
int setSketchStats(const char *key, SketchStats stats) {
  return store.set(key, reinterpret_cast<uint8_t *>(&stats),
                   sizeof(SketchStats), 0);
}
/*****************
       MAGSPOOF
 *****************/
...
void set_delay() {
  char *arg;
  arg =
      SCmd.next(); // Get the next argument from the SerialCommand object buffer
  ...
  if (arg != NULL) {
    ...
    if (ndelay < 1 || ndelay >= 1500) {
      if (debug) {
        Serial.println(
            "Error setting the command delay value must be between 1-1500");
        ndelay = 1000;
      }
      Serial.println("ERROR");
      return;
...
void test_host() {
  ...
  // test command: 0x00 0xA4 0x04 0x00 0x0E 0x32 0x50 0x41 0x59 0x2E 0x53 0x59
  ...
  if (arg != NULL) {
    ...
    if (n_host < 1 || n_host > 99) {
      if (debug) {
        Serial.println(
            "Error setting the command, test_host value must be between 1-99");
        nhost = 01;
      }
      Serial.println("ERROR");
      return;
...
void setup() {
  ...
  if (debug) {
    ...
    Serial.println("Welcome to the BomberCat CLI " + String(fwVersion, 1) +
                   "v\n");
  ...
  SCmd.setDefaultHandler(
      unrecognized); // Handler for command that isn't matched  (says "What?")
