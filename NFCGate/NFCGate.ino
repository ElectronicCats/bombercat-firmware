 ...
 * `status`, `stop`, `identify`. If RELAY_AUTOSTART is set (arduino_secrets.h)
 * the relay also starts on boot from the persisted/secrets config, so it still
 * runs standalone.
 *
...
#define BOMBERCAT_FW_VERSION "0.9.8"
...
// blowing past the CLI's run window. WiFiNINA's default connTimeout (0) leaves
// it to the NINA firmware's long default. Keep the worst-case budget aligned
// with the CLI:
...
// only kicks the bring-up off (returns
// `+OK accepted` at once) and loop() advances it one phase per iteration, so
// the serial REPL is serviced throughout and a slow/stuck phase can never wedge
// the device. Progress is published over `status` (idle -> connecting ->
// relaying | error). This replaces the old synchronous runRelay() that blocked
// loop() for the whole WiFi+NFC+TCP+SYN sequence.
...
// polling from here on. Static WiFi.begin() itself is issued lazily on the
// first Wifi tick so this returns immediately.
...
// no-op unless a bring-up is in progress. Each phase either progresses, or
// fails into Phase::Error with g_detail set. Only the WiFi phase is spread
// across iterations (its 20 s deadline is the long one); NFC and TCP are single
// bounded calls, so they occupy one iteration each but never hang the REPL for
// long.
...
// or a short reason it can't start (reported as -ERR). Never blocks on
// WiFi/NFC.
...
// (`bombercat device list` / `-d <id>`); those numbers come from USB identity,
// so this is what ties one to the physical board. Armed by the `identify`
// command and driven from loop() — never blocking, so it is safe to ask a
// relaying device to identify itself mid-session.
...
static void startIdentify() {
  ...
  if (g_identifyUntil == 0)
    g_identifyUntil = 1;     // 0 is the "idle" sentinel
...
void setup() {
  ...
  // block Serial.print mid-transaction on the relay hot path — added latency
  // for no benefit once the relay is validated. Raise it at runtime with the
  // CLI
  // (`loglevel 4`) or `bombercat monitor` when diagnosing. Capture is a
  // separate sink (`capture on`) and is unaffected by this level.
...
void loop() {
  ...
  driveBringup(); // advance any in-progress bring-up (no-op when not
                  // connecting)
