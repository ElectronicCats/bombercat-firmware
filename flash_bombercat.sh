...
# Librerías que no están en el Library Manager: se instalan desde git.
# Formato: "Nombre|URL del repositorio"
# OJO: NO añadas aquí ElectronicCats/Preferences. El fork de WiFiNINA que usa
# el BomberCat ya trae esa clase (WiFiPreferences.h) y tener ambas provoca
# "multiple definition of Preferences::..." al enlazar.
GIT_LIBRARIES=(
  "SerialCommand|https://github.com/kroimon/Arduino-SerialCommand.git"
)

# Rutas: los sketches viven en la raíz del repositorio, junto a este script.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FIRMWARE_DIR="$SCRIPT_DIR"
# Librería local del repositorio (BomberCatCore), necesaria para NFCGate.
CORE_LIB_DIR="$SCRIPT_DIR/core"
...
ensure_core() {
  ...
  # Sin --overwrite: si ya hay configuración, se respeta (no se borran otros
  # board_manager.additional_urls del usuario: esp32, Heltec, etc.).
...
ensure_libraries() {
  ...
  # Librerías que solo existen en git (SerialCommand, Preferences de Electronic Cats)
  local entry url
  for entry in "${GIT_LIBRARIES[@]}"; do
    lib="${entry%%|*}"; url="${entry#*|}"
    if arduino-cli lib list 2>/dev/null | grep -qi "$lib"; then
      ok "Librería '$lib' presente."
      continue
    fi
    info "Instalando '$lib' desde git..."
    arduino-cli config set library.enable_unsafe_install true >/dev/null 2>&1 || true
    if arduino-cli lib install --git-url "$url"; then
      ok "Librería '$lib' instalada."
    else
      err "No se pudo instalar '$lib' automáticamente."
      printf '\n%s    Cómo instalar '\''%s'\'' manualmente (no está en el Library Manager):%s\n' "$C_BLD" "$lib" "$C_RST" >&2
      ...
      printf '       git clone %s ~/Arduino/libraries/%s)\n\n' "$url" "$lib" >&2
      missing+=("$lib")
    fi
  done
...
# --library expone la librería BomberCatCore del propio repositorio (core/),
# que no está en el Library Manager y usan los sketches como NFCGate.
COMPILE_ARGS=(--fqbn "$FQBN" --output-dir "$BUILD_DIR")
[[ -f "$CORE_LIB_DIR/library.properties" ]] && COMPILE_ARGS+=(--library "$CORE_LIB_DIR")
